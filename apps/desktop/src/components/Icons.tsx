// Minimal line icons in the spirit of SF Symbols (1.6px round strokes).
export type IconName =
  | "home" | "alert" | "terminal" | "path" | "search" | "chip" | "box" | "activity" | "network" | "gear" | "drive"
  | "sparkles" | "wrench" | "git" | "key" | "clock" | "diff" | "sliders" | "check" | "x" | "chevron" | "info" | "undo"
  | "play" | "folder" | "refresh" | "shield" | "lock" | "heart" | "eye";
...
export function Icon({ name, size, className, title }: { name: IconName; size?: number; className?: string; title?: string }) {
  const s = size ?? 16;
  return (
    <svg className={`icon ${className ?? ""}`} width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round" aria-hidden={title ? undefined : true} role={title ? "img" : undefined}>
