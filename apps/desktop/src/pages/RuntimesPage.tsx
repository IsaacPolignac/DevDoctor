...
export function RuntimesPage({ refreshKey }: { refreshKey: number }) {
  ...
  return (
    <div className="page">
      ...
      <Card>
        <KeyValue rows={[["python", python.python ? <span className="mono selectable">{shortenHome(python.python.path, home)} · {python.python.version ?? ""}</span> : t("not found")], ["python3", python.python3 ? <span className="mono selectable">{shortenHome(python.python3.path, home)} · {python.python3.version ?? ""}</span> : t("not found")], ["pip3", python.pip3 ? <span className="mono selectable">{shortenHome(python.pip3.path, home)} · {python.pip3.version ?? ""}</span> : t("not found")], [t("Alias"), python.python_alias ? <span className="mono">{t("python=")}{python.python_alias}</span> : "—"]]} />
      </Card>
      <div style={{ height: 10 }} />
