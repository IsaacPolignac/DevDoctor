#!/usr/bin/env bash
# Verifies a rendered deliverable against BRIEF.md §2/§6/§8: 1920x1080, 30 fps, 60.00 s (1800 frames),
# audio -14 LUFS integrated (±1), true peak <= -1 dBTP.
set -euo pipefail
f="${1:-renders/purepeptide-manifeste-60s.mp4}"
...
echo "$a"
fail=0
grep -q "width=1920" <<<"$v" && grep -q "height=1080" <<<"$v" || { echo "FAIL size"; fail=1; }
grep -q "r_frame_rate=30/1" <<<"$v" || { echo "FAIL fps"; fail=1; }
grep -q "nb_read_frames=1800" <<<"$v" || { echo "FAIL frame count"; fail=1; }
awk -v d="$d" 'BEGIN{exit !(d>=59.99 && d<=60.05)}' || { echo "FAIL duration"; fail=1; }
...
[ $fail -eq 0 ] && echo "PASS: 1920x1080 · 30 fps · 1800 frames · ${d}s · ${I} LUFS · TP ${TP} dBTP"
exit $fail
