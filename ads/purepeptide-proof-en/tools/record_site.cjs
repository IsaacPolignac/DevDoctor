// Real site test of https://purepeptide.care (EN), recorded frame by frame at 30 fps.
...
(async () => {
  ...
  await pg.evaluateOnNewDocument((css) => {
    const add = () => {
      if (window !== window.top) return; // not inside payment iframes
    ...
    const hideRecon = () => {
      document.querySelectorAll(".cartdw *").forEach((e) => {
        if (e.children.length && /reconstitu/i.test(e.textContent) && e.parentElement && /Code promo|Promo code|Subtotal|Sous-total/i.test(e.parentElement.textContent) && !/Code promo|Promo code|Subtotal|Sous-total/i.test(e.textContent)) e.classList.add("pp-rec-hide");
      });
    };
  ...
  await pg.goto("https://purepeptide.care/?lang=en", { waitUntil: "networkidle2", timeout: 90000 });
  ...
  const marks = [];
  const mark = (name) => { console.log("mark", name, n); marks.push({ name, frame: n, t: +(n / FPS).toFixed(3) }); };
  ...
  const click = async (sel, nav) => {
    ...
    if (nav) {
      const before = pg.url();
      await pg.click(sel);
      const tries = nav === "maybe" ? 12 : 120;
      for (let k = 0; k < tries && pg.url() === before; k++) await sleep(250); // prerendered navigations fire no nav event
      if (pg.url() === before) {
        if (nav !== "maybe") throw new Error("click did not navigate: " + sel);
        await sleep(1500); // AJAX add-to-cart: let the cart update (not recorded)
      }
      for (let k = 0; k < 80; k++) { try { if ((await _eval(() => document.readyState)) === "complete") break; } catch (e) {} await sleep(250); }
      await sleep(2500); // let images/reveals settle (not recorded)
      await cdp.send("Animation.setPlaybackRate", { playbackRate: 1 / SLOW });
  ...
  mark("home");
  c = await pg.evaluate(() => { const a = [...document.querySelectorAll("a")].find((a) => /Explore the catalog/.test(a.textContent) && a.getBoundingClientRect().top > 0 && a.getBoundingClientRect().top < 900); const r = a.getBoundingClientRect(); a.id = "pp-rec-cta"; return { x: r.left + r.width / 2, y: r.top + r.height / 2 }; });
  ...
  await move(c.x, c.y, 0.6); await hold(0.2); mark("add-click"); await click(".pd__buy .add-btn", "maybe");
