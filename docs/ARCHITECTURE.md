...
```
                    ...
                    │ detectors      │   │ fixers         │  │ macos (Platform impl)│
                    └────────────┬───┘   └───────┬────────┘  └───┬──────────────────┘
                                 │               │               │
                         ┌───────▼───────────────▼───────────────▼───────┐
...
- `transaction`: `Transaction`, `Operation`, `MutationPolicy` (allowed roots, forbidden
  ...
  `write_file`, `delete_file`, `delete_dir`, `clear_dir`, `stop_process`, `run_command`) and
  `TransactionManager` (apply with automatic rollback, manual rollback).
...
services, and `sysctl`/`sw_vers` for system information. `FakePlatform` serves tests.
