...
# Needs the GitHub release (source tarball and Windows zip) to exist. The Release workflow runs
# it automatically after publishing and commits the result to main.
set -eu
...
# `-i.bak` works with both BSD (macOS) and GNU (Linux CI) sed.
sed -i.bak -e "s|url \".*archive/refs/tags/v.*\.tar\.gz\"|url \"$src_url\"|" -e "s|sha256 \".*\"|sha256 \"$src_sha\"|" Formula/devdoctor.rb && rm -f Formula/devdoctor.rb.bak
